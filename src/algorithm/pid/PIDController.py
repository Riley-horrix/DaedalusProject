...
from lib.NeuralPlane.algorithms.pid.rollController import RollController
from lib.NeuralPlane.algorithms.pid.pitchController import PitchController
from lib.NeuralPlane.algorithms.pid.yawController import YawController
from lib.NeuralPlane.algorithms.pid.TECS import TECS
from lib.NeuralPlane.algorithms.pid.L1Controller import L1Controller
from lib.NeuralPlane.algorithms.utils.utils import wrap_2PI, wrap_PI

from src.algorithm.pid.BasicPIDNav import BasicPIDNav

device = "cuda:0"

class Controller:
    def __init__(self, airspeed_min=100, airspeed_max=2300, dt=0.02, n=1, device=device):
        self.basic_nav = BasicPIDNav(dt, n, device)
    ...
    def update_simple_waypoint(self, env, target_pos):
        """
        target_pos: Tensor [n, 3] (North, East, Alt)
        """
        npos, epos, alt = env.model.get_position()
        current_pos = torch.hstack((npos.reshape(-1,1), epos.reshape(-1,1), alt.reshape(-1, 1))) # [n, 2]
        _, _, yaw = env.model.get_posture()

        roll_dem, pitch_dem = self.basic_nav.get_navigation_commands(
            current_pos,
            target_pos,
            yaw.reshape(-1,1),
            alt.reshape(-1,1)
        ...
        self.roll_dem = roll_dem
        self.pitch_dem = pitch_dem

        self.stabilize_roll(env)
        self.stabilize_pitch(env)

        # Just set 60% throttle for testing
        self.throttle_dem = torch.ones((self.n, 1), device=self.device) * 0.6