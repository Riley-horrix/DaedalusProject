...
JSONBackend::JSONBackend(const std::string& key): Configurable(key) {
    ...
    if (inet_pton(AF_INET, SERVER_ADDR.c_str(), &serverAddr.sin_addr) != 1) {
        error("Failed to convert server network address '%s'", SERVER_ADDR.c_str());
    }

    info("JSONBackend connected to address %s on port %" PRIu16, SERVER_ADDR.c_str(), SERVER_PORT);
}
...
std::unique_ptr<PhysicsBackend::Telemetry> JSONBackend::iterate(const std::unique_ptr<Control> ctrl) {
    ...
    for (int i = 0; i < pwmLen; i++) {
        ...
        control.pwm[i] = htons(static_cast<uint16_t>(std::round(ctrl->pwm[i] * 500) + 1500));
    }
    
    // Send the control packet over the UDP port to the physics backend
    ssize_t bytesSent = sendto(sockfd, &control, sizeof(control), 0, reinterpret_cast<sockaddr*>(&serverAddr), sizeof(serverAddr));
    ...
    while(Utils::micros() < start + timeout) {
        ...
        ssize_t receivedBytes = recvfrom(sockfd, &telemBuffer, BUFFER_SIZE - 1, MSG_DONTWAIT, NULL, NULL);

        // Received an error
        if (receivedBytes <= 0) {
            if ((receivedBytes < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) || receivedBytes == 0) {
                // This is fine and just means physics backend has not started yet or no data
                if (usleep(recvTimeout) != 0) {
                    stl_warn(errno, "Failed to usleep while waiting for physics backend");
                    return nullptr;
                }
                continue;
            }
...
bool JSONBackend::validateAndGetJson(double* value, const std::string& field, json j) {
...
bool JSONBackend::validateAndGetJsonArr(double* values, const std::string& field, json j, const unsigned int len) {
...
void JSONBackend::configure(void) {
    ...
    SERVER_PORT = static_cast<uint16_t>(confNum("port", static_cast<double>(SERVER_PORT)));
}